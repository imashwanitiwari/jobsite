        ...
        if($this->api->authentication())
            {
                if(isset($_POST['OP_ID'],$_POST['START_DATE'],$_POST['END_DATE'],$_POST['BY'], $_POST['BY_ID']))//validating all reqiured feild either recieved or not
                {
                    if($_POST['BY']=='OP_ID')
                        {
                        $response_count=$this->where->check_where_num("invoice","DATE BETWEEN '".$_POST['START_DATE']."' AND '".$_POST['END_DATE']."' AND OP_ID=".$_POST['OP_ID']);
                        $data = $this->where->select_where("invoice","DATE BETWEEN '".$_POST['START_DATE']."' AND '".$_POST['END_DATE']."' AND OP_ID=".$_POST['OP_ID'],"*");

                        }
                    else {
                        $response_count=$this->where->check_where_num("invoice","DATE BETWEEN '".$_POST['START_DATE']."' AND '".$_POST['END_DATE']."' AND OP_ID=".$_POST['OP_ID']." AND ".$_POST['BY']."=".$_POST['BY_ID']);
                        $data = $this->where->select_where("invoice","DATE BETWEEN '".$_POST['START_DATE']."' AND '".$_POST['END_DATE']."' AND OP_ID=".$_POST['OP_ID']." AND ".$_POST['BY']."=".$_POST['BY_ID'],"*");
                    }
                    if($response_count)
                    {
