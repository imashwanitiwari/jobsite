...
;(function( $, window, document, undefined ) {
	...
	Wizard.prototype = {
		defaults: {
			...
			nextButtonClass: 'btn danger', 
			prevButtonClass: 'btn left', 
		...
		init: function() {				
			...
			this.$elem.addClass('wizard-form');
		...
		initSteps: function() {
			...
			this.wizardNav.find('.wizard-progress')
				.animate({
		...
		goTo: function(stepNumber) {
			if(stepNumber !== this.data.activeStep) {
				...
				this.steps.eq(this.data.activeStep).data('done', true).fadeOut('fast', function() {
					...
					self.wizardNav.find('.wizard-progress')
						.animate({
		...
		_buildNavigation: function() {
			...
			this.steps.each(function(index, el) {
				...
				$ul.append(
					...
					.append(
						$span.clone().addClass('wizard-label wiz_state_'+(index + 1)).text(val.length != 0? val.text() : ('Step ' + (index + 1).toString()))
					)
				);
			});
			...
			this.wizardNav = $('<div></div>')
				.addClass('wizard-nav')
				.append($ul)
				.append($span.clone().addClass('wizard-progress'))
